 ...
 **/
(function($) {
    // This is a local copy of JQuery-UI to avoid conflicts with the system console's version.
    ...
    * Copyright 2015 jQuery Foundation and other contributors; Licensed MIT */
    ...
    $(function() {
        var lastValue = undefined,
            filter = $("#filter"),
            defaultValue = filter.attr("data-default-value"),
            autoCompletedTypes = {
                // order is used to sort the entries in the autocompletion dialog
                "mappings" : {hint: "The number of subsequent resource to model mappings initiated by a model", order:40},
                "mappableFields" : {hint: "The number of model fields that can be mapped from a resource", order:50},
                "lazyFields" : {hint: "The number of lazy-loaded model fields", order:60},
                "greedyFields" : {hint: "The number of greedy-loaded model fields", order:70},
                "instantiations" : {hint: "The number of times a model was instantiated", order:30},
                "averageMappingDuration" : {hint: "The average duration of a single resource to model mappings (in milliseconds)", order:80},
                ...
                "cacheHits" : {hint: "The number of times the model was found in a resource model cache, e.g. the NEBA request-scoped model cache.", order:120},
            ...
            valid = function() {
                filter.css("background", "url('assets/documentation/modelstatistics/valid.png') no-repeat 4pt center");
                filter.valid = true;
            },
            invalid = function() {
                filter.css("background", "url('assets/documentation/invalid.png') no-repeat 4pt center");
                filter.valid = false;
            },
            ...
            processFilterExpression = function(force) {
                ...
                try {
                    var expression = parser.parse(val);
                    valid();
            ...
            visualize = function(filterExpression) {
                var filter = function (e) {
                    var entire = e;
                    return function(modelData) {
                        return filterExpression ? eval(filterExpression) : true;
                    };
                };
                ...
                var visualizeData = function (data) {
                        ...
                        entire = {
                            modelData: {
                                add: function(key, value) {
                                    ...
                                    var avgKey = (key + "Avg");
                                    this[sumKey] = this[sumKey] ? this[sumKey] + value : value;
                                    this[avgKey] = this[avgKey] ? (this[avgKey] * this[countKey] + value) / (this[countKey] + 1) : value;
                                    this[countKey] = this[countKey] ? this[countKey] + 1 : 1;
                                }
                            }
                        };
                    ...
                    var star = d3.starPlot()
                        .width(width)
                        .properties(fields.filter(function(field) { return field.label }).map(function(field) { return field.name}))
                        .scales(fields.filter(function(field) { return field.label }).map(function(field) {
                    ...
                    d3.selectAll(".wrapper").transition().style("opacity", "1")
                };

                // Fade out and remove any previously rendered diagrams, then
                // create and fade-in the new diagrams.
                var existingPlots = d3.selectAll(".wrapper");
                if (existingPlots.empty()) {
                    d3.json("assets/documentation/modelstatistics/data.json", visualizeData);
                } else {
                    var n = 0;
        ...
        // Process filter changes, do not actually submit the form.
        $("form").submit(function() {processFilterExpression(); return false;});

        // Disable menu dividers: "-" is an arithmetic symbol, but
        // is interpreted as a menu divider by default.
        ...
        // Customized autocomplete dropdown rendering: Render the
        // suggestion, plus an additional help text, if available.
        $.widget( "ui.autocomplete", $.ui.autocomplete, {
            _renderItem: function( ul, item ) {
                var li = $("<li>" +
                "<strong>" + item.label.substr(0, item.highlightLength) + "</strong>" +
                item.label.substr(item.highlightLength) +
                (item.hint ? " <span> &ndash; " + item.hint + "</span>" : "") +
                "</li>")
        ...
        // By default, automatically apply any change to the filter field.
        filter.autoApply = true;

        filter.autocomplete({
            source: function(request, response ) {
                try {
                    parser.parse(request.term);
                    valid();
                    response();
                } catch (e) {
                    invalid();
                    if(e.expected) {
                        ...
                        suggestions = e.expected
                            .filter(function(element) {
                                // Include expected elements that have a description,
                                ...
                                var completion = autoCompletedTypes[expected.value || expected.description];
                                // Provide the auto completion data (label, value) and additional data for the customized
                                // auto completion suggestion rendering
                                return $.extend({
                                    order: completion ? completion.order : 1000,
            ...
            open: function () {
                // Do not automatically apply the filter while the user is choosing autocomplete options
                filter.autoApply = false;
            },
            close: function() {
                // Re-enable automatic updates and apply the recent most changes
                filter.autoApply = true;
