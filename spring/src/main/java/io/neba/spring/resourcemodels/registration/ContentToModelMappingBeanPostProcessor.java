 ...
 * using a thread-local stack of {@link #push(ResourceModelFactory.ContentToModelMappingCallback)} (ContentToModelMappingCallback) registered callbacks}.
 *
 * @see SpringModelRegistrar
 */
class ContentToModelMappingBeanPostProcessor implements BeanPostProcessor {
    private final ThreadLocal<Deque<ResourceModelFactory.ContentToModelMappingCallback<Object>>> mappingCallbackThreadLocal = ThreadLocal.withInitial(LinkedList::new);
    ...
    public final Object postProcessBeforeInitialization(@Nonnull Object bean, String beanName) throws BeansException {
        ...
        ResourceModelFactory.ContentToModelMappingCallback<Object> contentToModelMappingCallback = mappingCallbackThreadLocal.get().peek();
    ...
    void push(ResourceModelFactory.ContentToModelMappingCallback<Object> callback) {
        mappingCallbackThreadLocal.get().addFirst(callback);
    }

    void pop() {
        Queue<ResourceModelFactory.ContentToModelMappingCallback<Object>> stack = mappingCallbackThreadLocal.get();
        stack.remove();
        if (stack.isEmpty()) {
